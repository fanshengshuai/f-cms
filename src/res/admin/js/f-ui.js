/*f-ui.js v2.3 date:9:53 2015-07-17 by:guojunhui*/
...
/*添加收藏
<a title="收藏本站" href="javascript:;" onClick="addFavoritepage('f-ui前端框架','http://www.f-ui.net/');">收藏本站</a>
*/
/*收藏主站*/
...
/*返回顶部*/
var $backToTopEle=$('<a href="javascript:void(0)" class="f-ui-iconfont toTop" title="返回顶部" alt="返回顶部" style="display:none">&#xf0023;</a>').appendTo($("body")).click(function(){
...
$(function(){
	...
	/*tag标签*/
	var time1;
	$(".f-ui-tags-lable").show();
	$(".f-ui-tags-input").val("");
	$(document).on("blur",".f-ui-tags-input",function(){
		time1 = setTimeout(function(){
			$(this).parents(".f-ui-tags").find(".f-ui-tags-list").slideUp();
		}, 400);
	});
	$(document).on("focus",".f-ui-tags-input",function(){
		clearTimeout(time1);
	});
	$(document).on("click",".f-ui-tags-input",function(){
		$(this).find(".f-ui-tags-input").focus();
		$(this).find(".f-ui-tags-list").slideDown();
	});
	function gettagval(obj){
		var str ="";
		var token =$(obj).parents(".f-ui-tags").find(".f-ui-tags-token");
		//alert(token.length)
		if(token.length<1){
			$(obj).parents(".f-ui-tags").find(".f-ui-tags-val").val("");
			return false;
		}
		for(var i = 0;i< token.length;i++){
			str += token.eq(i).text() + ",";
			$(obj).parents(".f-ui-tags").find(".f-ui-tags-val").val(str);
		}
	}
	$(document).on("keydown",".f-ui-tags-input",function(event){
		$(this).next().hide();
		var v = $(this).val().replace(/\s+/g, "");
		var reg=/^,|,$/gi;
		v=v.replace(reg,"");
		v=$.trim(v);
		var token =$(this).parents(".f-ui-tags").find(".f-ui-tags-token");
		if(v!=''){
			if(event.keyCode==13||event.keyCode==108||event.keyCode==32){
				$('<span class="f-ui-tags-token">'+v+'</span>').insertBefore($(this).parents(".f-ui-tags").find(".f-ui-tags-iptwrap"));
				$(this).val("");
				gettagval(this);
			}
		}else{
			if(event.keyCode==8){
				if(token.length>=1){
					$(this).parents(".f-ui-tags").find(".f-ui-tags-token:last").remove();
					gettagval(this);
				}
				else{
					$(this).parents(".f-ui-tags").find(".f-ui-tags-lable").show();
					return false;
				}
	...
	$(document).on("click",".f-ui-tags-has span",function(){
		var taghasV = $(this).text();
		taghasV=taghasV.replace(/(^\s*)|(\s*$)/g,"");
		$('<span class="f-ui-tags-token">'+taghasV+'</span>').insertBefore($(this).parents(".f-ui-tags").find(".f-ui-tags-iptwrap"));
		gettagval(this);
		$(this).parents(".f-ui-tags").find(".f-ui-tags-input").focus();
	});
	$(document).on("click",".f-ui-tags-token",function(){
		var token =$(this).parents(".f-ui-tags").find(".f-ui-tags-token");
		var it = $(this).parents(".f-ui-tags");
		$(this).remove();
		switch(token.length){
			case 1 : it.find(".f-ui-tags-lable").show();
			break;
		}
		var str ="";
		var token =it.find(".f-ui-tags-token");
		//alert(token.length)
		if(token.length<1){
			it.find(".f-ui-tags-val").val("");
			return false;
		}
		for(var i = 0;i< token.length;i++){
			str += token.eq(i).text() + ",";
			it.find(".f-ui-tags-val").val(str);
		}		
	});
});

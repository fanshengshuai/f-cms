...
class AdminMemberCtrl extends AdminAbstractCtrl {
    public function indexAction() {
        $m = new FTable("user");
        $ainfo = $m->order(array('id' => 'desc'))->select();
        $count = $m->count();
    ...
    public function member_startAction() {
        $id = $_POST['id'];
        $m = new FTable("user");
        if ($m->where("id = $id")->update(array('status' => 1))) {
            echo 1;
        } else {
            echo 0;
        }
    ...
    public function member_stopAction() {
        $id = $_POST['id'];
        $m = new FTable("user");
        if ($m->where("id = $id")->update(array('status' => 0))) {
            echo 1;
        } else {
            echo 0;
        }
...
//        $table = new FTable("postcard");
        $adinfo = $m->fields("id, user_name, nick_name,phone, email")->find($id);
//        $userinfo = $table->where('user_id=' . $id)->find();


//        $adinfo['id'] = $adinfo1['id'];
...
//        $adinfo['content1'] = $userinfo['content1'];
    ...
    public function exportAction() {
        require_once WEB_ROOT_DIR . "/lib/flib/src/Excel.php";
        $list = FSession::get('excel_data');
        $row = array();
        $row[0] = array('序号', '用户名', '昵称', '手机', '邮箱');
        $row[1] = $list;
        $xls = new Excel_XML('UTF-8', false, $list['user_name']);
        $xls->addArray($row);
        $xls->generateXML($list['user_name']);
    }
...
//            $adinfo[$key]['content1'] = $userinfo['content1'];
    ...
    public function do_pwdAction() {
        ...
        if ($m->where("id = $id")->update(array('password' => $password))) {
            $arr['status'] = 'y';
            $arr['info'] = '更新成功';
        } else {
            $arr['status'] = 'n';
    ...
    public function member_hyAction() {
        $id = $_POST['id'];
        $m = new FTable("user");
        if ($m->where("id = $id")->update(array('deleted' => 1))) {
            echo 1;
        } else {
            echo 0;
        }
    ...
    public function changeAction() {
        ...
        foreach ($card as $key => $v) {
            $user = $table->where('id=' . $v['user_id'])->find();
            $apply = $table->where('id=' . $v['apply_user'])->find();
    ...
    public function user_msgAction() {
        ...
        foreach ($message as $key => $v) {
            if ($v['type']) {
                ...
                $user = $member->where('id=' . $v['sender'])->find();
                $apply = $member->where('id=' . $v['recipient'])->find();
                $data[$key]['user_name'] = $user['username'];
                $data[$key]['user_nickname'] = $user['nickname'];
                $data[$key]['apply_name'] = $apply['username'];
                $data[$key]['apply_nickname'] = $apply['nickname'];
